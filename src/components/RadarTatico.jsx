...
const RadarTatico = ({
  ...
  const refresh = () => {
    ...
    const activeEnemies = Object.values(ships).filter(
  ...
  const handleProximityChoice = (delta) => {
    ...
    const remaining = engagementResult.filter(r => r.enemyId !== pendingChoice.enemyId);
  ...
  const renderRadar = () => {
    ...
    return (
      <svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg" className="rt-svg">
        <defs>
        ...
        <line x1="50" y1="2" x2="50" y2="98" stroke="rgba(255,255,255,0.04)" strokeWidth="0.3" />
        <line x1="2" y1="50" x2="98" y2="50" stroke="rgba(255,255,255,0.04)" strokeWidth="0.3" />
        ...
        <g>
          <circle cx="50" cy="50" r="2.5" fill={radarThemeColors.player} className="rt-player-pulse" />
          <polygon points="50,46.5 52.5,52 50,50.5 47.5,52" fill={radarThemeColors.player} opacity="0.9" />
          {playerShip?.isDerrapando && (
            <circle cx="50" cy="50" r="4" fill="none" stroke="#d89329" strokeWidth="0.5" strokeDasharray="1 1" className="rt-derrap-ring" />
          )}
        </g>
      ...
      <div className="rt-radar-wrap">
        ...
        <div className="rt-speed-hud">
          ...
          <div className="rt-speed-bar-bg">
            <div
              className={`rt-speed-bar-fill ${speedPct > 80 ? "high" : speedPct > 40 ? "mid" : "low"}`}
              style={{ width: `${speedPct}%` }}
            />
          </div>
          {isDerrapando && <div className="rt-derrap-badge">⚠ DERRAPANDO −20%</div>}
        </div>
      </div>
            ...
            <div key={enemy.id} className="rt-enemy-row">
              <div className="rt-enemy-left">
                ...
                <span className="rt-enemy-speed">
                  ...
                  {enemy.isDerrapando && <span className="rt-skid-tag"> DERRAP</span>}
                </span>
              </div>
              <div className="rt-enemy-right">
                <span className="rt-prox-badge" style={{ borderColor: color, color }}>
                  P{prox} {proximityLabel(prox)}
                </span>
                {mods.advantageBonus > 0 && <span className="rt-mod-tag green">+{mods.advantageBonus}d VTG</span>}
                {prox === 4 && <span className="rt-mod-tag yellow">PREC ÷2</span>}
                {prox === 5 && <span className="rt-mod-tag red">BLOQUEADO</span>}
                {isMestre && (
                  <div className="rt-master-enemy-btns">
                    <button className="rt-mini-btn" onClick={() => handleRollEnemySpeed(enemy.id)}>🎲 VEL</button>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="rt-engagement-popup">
          ...
          <div className="rt-eng-body">
            ...
            <div className="rt-eng-speeds">
              <span className="rt-eng-spd ally">Jogadores: {pendingChoice.playerSpeed}</span>
              <span className="rt-eng-vs">VS</span>
              <span className="rt-eng-spd enemy">Inimigo: {pendingChoice.enemySpeed}</span>
            </div>
            <div className={`rt-eng-winner ${pendingChoice.winner === "player" ? "ally" : "enemy"}`}>
              {pendingChoice.winner === "player" ? "⚡ JOGADORES VENCERAM A MANOBRA" : "⚡ INIMIGO VENCEU A MANOBRA"}
            </div>
            ...
            <div className="rt-eng-btns">
              <button className="rt-eng-btn closer" onClick={() => handleProximityChoice(-1)}>
                ◀ APROXIMAR (P−1)
              </button>
              <button className="rt-eng-btn further" onClick={() => handleProximityChoice(+1)}>
                AFASTAR (P+1) ▶
              </button>
            </div>
